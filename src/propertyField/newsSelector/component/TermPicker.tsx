...
import { IPropertyFieldNewsSelectorHostProps } from './IPropertyFieldNewsSelectorHost';
...
export interface ITermPickerProps {
  newsSelectorHostProps: IPropertyFieldNewsSelectorHostProps;
...
export default class TermPicker extends React.Component<ITermPickerProps, ITermPickerState> {
    ...
    return (<div className={styles.termSuggestion} title={termTitle}>
      <div>{term.name}</div>
      {
        ...
        term.termSet.indexOf(term.key) !== -1 ? (
          <div className={styles.termSuggestionSubTitle}>Term Set</div>
        ) : (
  ...
  private async onFilterChanged(filterText: string, tagList: IPickerTerm[]): Promise<IPickerTerm[]> {
    const { context, newsSelectorHostProps: NewsSelectorHostProps, allowMultipleSelections, isTermSetSelectable, disabledTermIds } = this.props;
